...
const outputDir = path.resolve(scriptDir, "../public/sounds/game-shot");
const voice = process.env.SCORE_CLIP_VOICE?.trim() || "Daniel (English (UK))";
const rate = process.env.SCORE_CLIP_RATE?.trim() || "168";

const CLIPS = [
  { slug: "game-shot", phrase: "Game shot!" },
  { slug: "game-shot-and-the-first-leg", phrase: "Game shot and the first leg." },
  { slug: "game-shot-and-the-match", phrase: "Game shot and the match." },
];
...
if (process.platform !== "darwin") {
  console.error("generate-game-shot-clips requires macOS `say` and `afconvert`.");
  process.exit(1);
}
...
console.log(`Done. Generated ${CLIPS.length} game shot clips in ${outputDir}`);
