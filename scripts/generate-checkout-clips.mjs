...
if (process.platform !== "darwin") {
  console.error("generate-checkout-clips requires macOS `say` and `afconvert`.");
  process.exit(1);
}

await mkdir(outputDir, { recursive: true });

generateClip("no-finish", "No Finish");

for (let remaining = 2; remaining <= CHECKOUT_MAX; remaining += 1) {
  generateClip(`you-require-${remaining}`, `You require ${remaining}`);
}

console.log(`Done. Generated ${CHECKOUT_MAX - 1} checkout clips in ${outputDir}`);
