...
import type { BaseballGameState, BaseballPlayerState } from "@/types/baseball";
    ...
    >
      ...
      <div className="x01-player-card__score-row">
        <div className="x01-player-card__side-stat x01-player-card__side-stat--left">
          {!isActive && player.lastVisitRuns != null ? (
            <div className="x01-player-card__inline-stat">
              ...
              <span className="x01-player-card__side-stat-value">
                {player.lastVisitHomeRun ? "HR" : `+${player.lastVisitRuns}`}
              </span>
            </div>
          ) : null}
        </div>
        ...
        <div className="x01-player-card__score-block">
          {!tightLayout ? (
            <span className="x01-player-card__score-label">Runs</span>
          ) : null}
          <p className="x01-player-card__score">{player.runs}</p>
        </div>
...
export function BaseballScoreboard({
  ...
  const currentTarget = getBaseballCurrentTarget(game);
