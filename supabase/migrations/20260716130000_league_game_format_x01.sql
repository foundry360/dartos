-- Collapse 501/301/701 league game formats into a single X01 option.
-- Starting score lives on leagues.rules (Game Rules), not game_format.
...
-- Preserve legacy starting scores into rules when migrating.
update public.leagues
set
  rules = case
    when rules is null
      or coalesce(rules->>'family', '') <> 'x01' then
      jsonb_build_object(
        'family', 'x01',
        'startingScore', game_format::integer,
        'startingRule', null,
        'finishingRule', null,
        'bustRule', null,
        'matchFormat', null,
    ...
    when rules->>'startingScore' is null
      or rules->>'startingScore' = 'null' then
      rules || jsonb_build_object('startingScore', game_format::integer)
    else
      rules
  end,
  game_format = 'x01'
where game_format in ('501', '301', '701');
...
begin
  ...
  if trimmed_game = '' then
    trimmed_game := null;
  elsif trimmed_game in ('501', '301', '701') then
    trimmed_game := 'x01';
  elsif trimmed_game not in (
    'x01',
    'cricket',
    'tactics',
    'mixed',
