-- Competition structure for leagues (Round Robin, Points, Ladder, Custom).

alter table public.leagues
  add column if not exists competition_format text;

alter table public.leagues
  drop constraint if exists leagues_competition_format_allowed;

alter table public.leagues
  add constraint leagues_competition_format_allowed
  check (
    competition_format is null
    or competition_format in ('round_robin', 'points', 'ladder', 'custom')
  );
