-- Switch league.format from game formats to league types.
...
set format = case lower(trim(coalesce(format, '')))
  when 'team' then 'team'
  when 'doubles' then 'doubles'
  when 'blind_draw' then 'blind_draw'
  when 'ladder' then 'ladder'
  when 'singles' then 'singles'
  when 'cricket' then 'singles'
  when 'tactics' then 'singles'
  when '201' then 'singles'
  when '301' then 'singles'
  when '501' then 'singles'
  when '701' then 'singles'
  when '' then null
  else 'singles'
end
...
alter table public.leagues
  ...
  check (
    format is null
    or format in ('singles', 'team', 'doubles', 'blind_draw', 'ladder')
  );
