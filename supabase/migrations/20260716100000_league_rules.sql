-- League match-play rules (JSONB) inherited by every scheduled match.
-- Expand game_format to include tactics and mixed.

alter table public.leagues
  add column if not exists rules jsonb;

comment on column public.leagues.rules is
  'League-level match play rules. Every scheduled match inherits these settings.';
