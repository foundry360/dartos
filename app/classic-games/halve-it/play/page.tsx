...
import { HalveItPlaySidebar } from "@/features/classic-games/components/HalveItPlaySidebar";
import {
  formatHalveItProgress,
...
export default function HalveItPlayPage() {
  ...
  const game = useHalveItStore((state) => state.game);
  const throwDart = useHalveItStore((state) => state.throwDart);
  const finishTurn = useHalveItStore((state) => state.finishTurn);
  ...
  useMatchGameOnAnnouncement({
    ...
    onAfterAnnounce: () => {
      ...
      announceHalveItRound(activeGame);
    },
  });
    ...
    <>
      ...
      <ScoringLayout
        ...
        boardHeader={
          <BoardGameTitle
            title="Halve-It"
            subtitle={formatHalveItProgress(game)}
          />
        }
        board={
          <Dartboard
