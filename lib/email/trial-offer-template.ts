...
export function getTrialOfferFirstName(displayName: string | null | undefined): string {
  const first = displayName?.trim().split(/\s+/)[0];
  return first || "there";
}

export function renderTrialOfferEmailHtml(input: {
  firstName: string;
  trialActivationLink: string;
...
<title>VectorOS ${days}-Day Free Trial</title>
<style>
...
<div style="display:none; max-height:0; overflow:hidden; opacity:0;">
  Unlock everything VectorOS has to offer with a free ${days}-day Club or Elite trial.
</div>
      ...
      <table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#141916;">
        ...
        <tr>
          <td align="center" style="padding:24px 40px 8px;">
            <a href="${trialActivationLink}" style="display:inline-block; background-color:#6f9e24; color:#0E1210; text-decoration:none; font-family:'Barlow Condensed',Arial,sans-serif; font-size:16px; letter-spacing:1px; text-transform:uppercase; font-weight:600; padding:14px 36px; border-radius:4px;">
              Start My Free Trial
            </a>
          </td>
        </tr>
        ...
        <tr>
          <td class="px" style="padding:28px 40px 40px; text-align:center; font-family:Helvetica,Arial,sans-serif; font-size:12px; color:#6E6A5F; line-height:1.6;">
            ...
            You're receiving this because you have a free VectorOS account.<br>
